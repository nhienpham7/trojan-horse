...
const pM   = new THREE.MeshStandardMaterial({ color: 0xe2dcd0, roughness: 0.95 })
...
const rodM = new THREE.MeshStandardMaterial({ color: 0xb0a898, roughness: 0.5  })
...
function buildGeometry(scene) {
  ...
  const op  = 0.6
  wall(scene, CAS_X,      cz0-T/2,         CAS_W+T*2,      T,            pM)
  wall(scene, CAS_X,      cz1+T/2,         CAS_W+T*2,      T,            pM)
  wall(scene, CAS_X-CAS_W/2-T/2, CAS_Z,   T,              CAS_D,        pM)
  wall(scene, cx1+T/2,    CAS_Z-op/2,      T,              (CAS_D-op)/2, pM)
  wall(scene, cx1+T/2,    CAS_Z+op/2,      T,              (CAS_D-op)/2, pM)
  box(scene, CAS_X-0.35, 0.65, CAS_Z, 0.06, 1.3, 0.06, rodM)
  box(scene, CAS_X+0.35, 0.65, CAS_Z, 0.06, 1.3, 0.06, rodM)
  // Right divider
...
// ── Painting placement (exact positions from original HTML) ───────
function placePaintings(scene, rendererRef) {
  ...
  return [
    ...
    P((BTH_X0+BTH_X1)/2-0.3,    EH,   BTH_ZS-WO, Math.PI,       '3c', 0.65, 0.90),
