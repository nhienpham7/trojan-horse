...
const SplitText = ({ text, delay = 0, animate = true, className = '' }: SplitProps) => (
  <span
...
type Props = { config: ExhibitionConfig }

export default function Landing({ config }: Props) {
  const { meta, theme } = config

  const [showContent, setShowContent] = useState(false)
  const [preloaderExited, setPreloaderExited] = useState(false)
  const [userInteracted, setUserInteracted] = useState(false)
  const [heroCacheKey] = useState(() => Date.now())
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--color-brand-cream', theme.brand.cream)
    root.style.setProperty('--color-brand-ink', theme.brand.ink)
    root.style.setProperty('--color-brand-accent', theme.brand.accent)
    root.style.setProperty('--color-brand-paper', theme.brand.paper)
    root.style.setProperty('--color-brand-gold', theme.brand.gold)
  }, [theme])
  ...
  const titleParts = meta.titleParts ?? meta.title.split(' ')
  ...
  return (
    <div className="h-screen w-full bg-brand-cream text-brand-ink grainy overflow-hidden selection:bg-brand-accent selection:text-brand-cream font-sans p-4 md:p-8">
          ...
          {!showContent && (
            ...
            >
              ...
              <div className="overflow-hidden">
                ...
                >
                  <SplitText text={meta.preloaderText} delay={0.25} />
                </motion.p>
              </div>
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch h-full">
              ...
              >
                <div className="flex items-center gap-4 mb-12">
                  ...
                  <div className="text-[10px] uppercase font-bold tracking-[0.3em] leading-tight text-brand-accent">
                    {meta.subtitleLines.map((line, i) => (
                      <div key={line}>
                        <SplitText text={line} animate={showContent} delay={0.25 + i * 0.13} />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <h1 className="text-6xl md:text-7xl font-display uppercase tracking-tighter mb-8 flex flex-col gap-2 leading-[1] projector-flicker">
                  {titleParts.map((part, i) => {
                    const isStroke = i === 1 && titleParts.length === 3
                    return (
                      <span
                        key={`${part}-${i}`}
                        className={`block ${isStroke ? 'text-stroke' : ''}`}
                        style={isStroke ? { WebkitTextStrokeColor: 'var(--color-brand-ink)' } : undefined}
                      >
                        <SplitText
                          text={part}
                          animate={showContent}
                ...
                <div className="space-y-4 max-w-xs">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm font-serif leading-relaxed italic opacity-80">
                      <SplitText text={meta.location} animate={showContent} delay={0.95} />
                    </p>
                  </div>
                </div>
              </motion.div>

              {meta.curationLines.length > 0 && (
                <motion.div
                  initial={{ opacity: 0 }}
                ...
                >
                  <p className="text-xs font-mono uppercase tracking-[0.2em] font-semibold opacity-70 ink-underline">
                    <SplitText text={meta.curationLines[0]} animate={showContent} delay={1.6} />
                  </p>
                  {meta.curationLines.length > 1 && (
                    <>
            ...
            <div className="lg:col-span-5 lg:-ml-4 relative flex items-center justify-center py-8">
              ...
              >
                ...
                >
                  <source src={`${meta.heroVideoUrl}?v=${heroCacheKey}`} type="video/mp4" />
                </video>
              </motion.div>
            </div>
            ...
            <div className="lg:col-span-3 flex flex-col justify-between py-4">
              ...
              >
                ...
                <div className="space-y-2">
                  {meta.artistsList.map((artist, i) => (
                    ...
                    >
                      <span className="text-[10px] font-mono text-brand-accent/40 group-hover:text-brand-accent transition-colors">
                        {String(i + 1).padStart(2, '0')}
                      </span>
                      <span className="text-lg md:text-xl font-serif text-brand-ink/80 group-hover:text-brand-ink transition-colors">
                        {artist}
                      </span>
                    </motion.div>
                  ))}
              ...
              >
                ...
                >
                  ...
                  <ArrowUpRight
                    className="relative z-10 group-hover:rotate-45 transition-transform duration-700"
                    size={18}
                  />
                </motion.button>
              </motion.div>
      ...
      <Gallery visible={galleryVisible} config={config} />
      {galleryVisible && (
        <motion.button
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
